...
function getWinners(array, getFinalscb) {
    ...
    getFinalscb(array).forEach(function(item) {
       ...
       } else if(item['Home Team Goals'] + item['Half-time Home Goals'] < item['Away Team Goals'] + item['Half-time Away Goals']) {
...
function getWinnersByYear(array, cbGetYears, cbGetWinners) {
   return cbGetYears(array, getFinals).map(function(item, index) {
       return `In ${item}, ${cbGetWinners(array, getFinals)[index]} won the world cup!`;
   })
}
...
function getAverageGoals(cbGetFinals) {
    ...
    let average = cbGetFinals.reduce(function(acc, item) {
       return (acc + item['Home Team Goals'] + item['Away Team Goals']);
   },0);
  return (average/cbGetFinals.length).toFixed(2)
}
